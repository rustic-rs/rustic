...
const SIZE_SQUARE_FACTOR: u32 = 256;
const MAX_COUNT: u32 = 10_000;
const MAX_AGE: Duration = Duration::from_secs(300);

pub fn size_limit_from_size(size: u64, default_size: u32) -> u32 {
    (size.integer_sqrt() as u32 * SIZE_SQUARE_FACTOR).clamp(default_size, MAX_SIZE)
}
...
impl<BE: DecryptFullBackend> Repacker<BE> {
    pub fn new(
        ...
        let size_limit = size_limit_from_size(total_size, default_size);
        Ok(Self {
            be,
            packer,
            size_limit,
