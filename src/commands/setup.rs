...
impl SetupCmd {
    fn inner_run(&self) -> Result<()> {
        ...
        _ = repo_table.insert("repository".to_string(), toml::Value::String(repository.clone()));
        ...
        if !exclude_if_present.is_empty() {
            _ = backup_table.insert(
                ...
                toml::Value::Array(exclude_if_present.iter().cloned().map(toml::Value::String).collect()),
            );
        }
        if !globs.is_empty() {
            _ = backup_table.insert(
        ...
        for source_paths in &sources {
            ...
            _ = snap_table.insert(
                "sources".to_string(),
                toml::Value::Array(source_paths.iter().cloned().map(toml::Value::String).collect()),
            );
        ...
        if let Some(ref ret) = retention {
            ...
            if ret.keep_daily > 0 {
                _ = forget_table.insert("keep-daily".to_string(), toml::Value::Integer(ret.keep_daily as i64));
            }
            if ret.keep_weekly > 0 {
                _ = forget_table.insert("keep-weekly".to_string(), toml::Value::Integer(ret.keep_weekly as i64));
            }
            if ret.keep_monthly > 0 {
                _ = forget_table.insert("keep-monthly".to_string(), toml::Value::Integer(ret.keep_monthly as i64));
            }
            if ret.keep_yearly > 0 {
                _ = forget_table.insert("keep-yearly".to_string(), toml::Value::Integer(ret.keep_yearly as i64));
            }
